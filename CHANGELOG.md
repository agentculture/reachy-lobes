...
- **Re-vendored the `ask-colleague` skill from colleague (now 1.7.0, up from the
  ...
  consumer-identifying Provenance clause (`reachy-lobes vendors from
...
- **Vendor the `outsource` skill** (`.claude/skills/outsource/`) from
  ...
  *different* engine/mind: `explore` (read-only investigation), `review` (a
  ...
  [#8](https://github.com/agentculture/reachy-lobes/issues/8).
...
- Expanded the clone-and-rename instructions in `CLAUDE.md`: added `README.md` to
  ...
  `reachy_lobes/cli/__init__.py`) rather than renaming by hand.
...
- SonarCloud now reports code coverage — added `relative_files = true` to
  ...
  `sonar.sources=reachy_lobes` (absolute / `.venv` paths were dropped
...
- **CI gates on the SonarCloud quality gate**
  ([issue #3](https://github.com/agentculture/reachy-lobes/issues/3)) —
...
- **Onboarded into the AgentCulture mesh** ([issue #1](https://github.com/agentculture/reachy-lobes/issues/1)).
...
- **Mesh identity**: `culture.yaml` (`suffix: reachy-lobes`,
