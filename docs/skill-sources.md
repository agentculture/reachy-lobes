...
reachy-lobes vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. reachy-lobes
...
#   - consumer-identifying prose: `guildmaster` → `reachy-lobes` (NOT
...
If a re-sync would lose a reachy-lobes adaptation, lift the change
...
#     → `which reachy-lobes vendors from guildmaster`
