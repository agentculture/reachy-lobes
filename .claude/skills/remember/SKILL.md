...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope reachy-lobes
...
scope:** the default personal scope is **private** (`--scope reachy-lobes
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `reachy-lobes`). Private records are served only to a recall in the same
