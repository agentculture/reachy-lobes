...
type: command
description: >
  Cross-repo + mesh communication from reachy-lobes: file tracked GitHub issues
  ...
  the next step lives outside reachy-lobes (a brief for a sibling-repo agent, a
  status ping for a Culture channel, or pulling an issue body + comments
  into context). Issue posts auto-sign with `- reachy-lobes (Claude)`; mesh
  messages are unsigned (the IRC nick is the speaker). Not for in-reachy-lobes
